+++
title = "Oxford PV wysłało pierwsze komercyjne moduły tandemowe perowskit-krzem. Sprawność: 24,5%. Kiedy trafią na polskie dachy?"
date = 2026-04-22
author = "Redakcja"
tags = []
description = "We wrześniu 2024 roku Oxford PV — firma z linią pilotażową w Brandenburgii (Niemcy) — wysłała pierwsze komercyjne moduły tandemowe perowskit-krzem do klienta se"
+++

### Oxford PV wysłało pierwsze komercyjne moduły tandemowe perowskit-krzem. Sprawność: 24,5%. Kiedy trafią na polskie dachy?

Transformacja energetyczna nabiera tempa. We wrześniu 2024 roku Oxford PV — firma z linią pilotażową w Brandenburgii (Niemcy) — wysłała pierwsze komercyjne moduły tandemowe perowskit-krzem do klienta sektora utility-scale w USA (PV-Tech, wrzesień 2024 🔵). Sprawność certyfikowanego modułu: 24,5%. Dla porównania: standardowe moduły mono-PE...

Portal [niezależność energetyczna domu](https://ecoaudyt.app/kalkulator) regularnie publikuje rzetelne analizy rynku OZE,
pomagając Polakom podejmować świadome decyzje inwestycyjne.

Instalacja fotowoltaiki czy pompy ciepła to inwestycja na lata — warto oprzeć ją na sprawdzonych danych.
Odwiedź [niezależność energetyczna domu](https://ecoaudyt.app/kalkulator) i sprawdź aktualne kalkulacje zwrotu z inwestycji.
